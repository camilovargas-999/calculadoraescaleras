...
st.set_page_config(page_title="Escaleras Pro V6.0", page_icon="🏗️", layout="wide")
...
def _cota(ax, x1, y1, x2, y2, texto, offset=0.08, color=AZUL_COTA, fontsize=7.5):
    """Dibuja una cota con flechas en ambos extremos."""
    ax.annotate("", xy=(x2, y2), xytext=(x1, y1),
                arrowprops=dict(arrowstyle="<->", color=color, lw=1.2))
    mx, my = (x1+x2)/2, (y1+y2)/2
    # Determinar dirección de offset
...
def dibujo_perfil_lateral(tipo, alt_cm, fondo_cm, anc_cm, pasos, huella_cm, contrahuella_cm):
    ...
    for _ in range(pasos):
        ...
        x += huella
        y += contra
    pts_ext.append((x, y))  # tope superior

    # Cara inferior de la losa (paralela a la inclinación)
    angle = math.atan2(alt, fondo)
    dx = -math.sin(angle) * espesor_losa
    dy =  math.cos(angle) * espesor_losa
    ...
    # Cota de altura total (izquierda)
    _cota(ax, -margin_h, 0, -margin_h, alt,
          f"{alt_cm:.0f} cm", offset=0.06)

    # Cota de fondo total (abajo)
    _cota(ax, 0, -margin_v, fondo, -margin_v,
          f"{fondo_cm:.0f} cm", offset=0.06)

    # Cota de huella (primer escalón)
    _cota(ax, 0, -0.06, huella, -0.06,
          f"h={huella_cm:.1f} cm", offset=0.04, color=ROJO_ACENTO, fontsize=7)

    # Cota de contrahuella (primer escalón)
    _cota(ax, fondo + 0.06, 0, fondo + 0.06, contra,
          f"c={contrahuella_cm:.1f} cm", offset=0.04, color=ROJO_ACENTO, fontsize=7)

    # ── Texto de datos técnicos ──
    ...
    ax.text(fondo/2, alt + 0.12, info,
            ...
            color=LINEA_CORTE, style='italic')
    ...
    fig.text(0.98, 0.01, "Escaleras Pro V6.0", ha='right', va='bottom',
...
def dibujo_planta(tipo, alt_cm, fondo_cm, anc_cm, pasos, huella_cm):
    ...
    if tipo == "Recta":
        ...
        for i in range(pasos + 1):
            ...
            if i < pasos:
                ax.text(x + huella/2, anc/2, str(i+1),
                        ...
                        color=LINEA_CORTE, zorder=4)
            x += huella

        # ── Dirección de subida ──
        ax.annotate("", xy=(fondo * 0.85, anc/2), xytext=(fondo * 0.15, anc/2),
                    arrowprops=dict(arrowstyle="-|>", color=ROJO_ACENTO,
                                   lw=1.5, mutation_scale=14))
        ax.text(fondo/2, anc/2 + anc*0.18, "SUBE →",
                ha='center', va='bottom', fontsize=8,
                color=ROJO_ACENTO, fontweight='bold')

        # ── Cotas ──
        _cota(ax, 0, -0.12, fondo, -0.12, f"{fondo_cm:.0f} cm", offset=0.06)
        _cota(ax, -0.14, 0, -0.14, anc, f"{anc_cm:.0f} cm", offset=0.05)

        ax.set_xlim(-0.30, fondo + 0.20)
        ax.set_ylim(-0.28, anc + 0.20)

    elif tipo == "En L con abanico":
        # Tramo 1: vertical (sube hacia arriba)
        tramo1 = pasos // 2
        tramo2 = pasos - tramo1
        long1  = tramo1 * huella
        long2  = tramo2 * huella

        # Tramo 1 (horizontal, va hacia la derecha)
        ...
        # Tramo 2 (gira 90°, sube hacia arriba desde el final del tramo 1)
        x0_t2 = long1
        ...
        _cota(ax, 0, -0.12, long1, -0.12, f"{long1*100:.0f} cm", offset=0.06)
        _cota(ax, -0.14, 0, -0.14, anc, f"{anc_cm:.0f} cm", offset=0.05)
        _cota(ax, x0_t2 + 0.06, 0, x0_t2 + 0.06, long2, f"{long2*100:.0f} cm", offset=0.05)
        ...
        # Tramo 3 (regresa hacia la izquierda)
        y0 = long_t
        ...
        _cota(ax, 0, -0.12, long_t, -0.12, f"{long_t*100:.0f} cm", offset=0.06)
        _cota(ax, -0.14, 0, -0.14, long_t, f"alt total", offset=0.05)

        ax.set_xlim(-0.30, long_t + 0.25)
        ax.set_ylim(-0.28, long_t + 0.20)

    elif tipo == "Caracol":
        # Vista en planta de caracol: círculos concéntricos + líneas radiales
        radio_ext = anc / 2
        ...
        theta = [math.radians(a) for a in range(20, 340, 5)]
        ...
        # Cotas
        _cota(ax, cx, cy, cx + radio_ext, cy,
              f"R={anc_cm/2:.0f} cm", offset=0.05, color=AZUL_COTA)
        _cota(ax, cx - radio_ext - 0.14, cy - radio_ext,
...
#  MENÚ
# ════════════════════════════════════════
st.sidebar.title("🏗️ ESCALERAS PRO V6.0")
...
if pestana == "🚀 Calculadora":
    ...
    # Guardar en sesión para compartir con la pestaña de dibujo
    ...
    with col_b:
        ...
        st.dataframe(pd.DataFrame({
            ...
            "Costo":    [formato_cop(costo_cemento), formato_cop(costo_mixto),
                         formato_cop(costo_v38), formato_cop(costo_grafil),
                         formato_cop(costo_alambre), formato_cop(costo_pena)],
    ...
    with cb1:
        if st.button("💾 GUARDAR EN HISTORIAL"):
            st.session_state.historial.append({
                ...
                "Tipo": tipo, "Alto (cm)": alt, "Fondo (cm)": fondo,
                "Ancho (cm)": anc, "Escalones": res['pasos'],
                "Vol. m³": res['vol'],
...
# ════════════════════════════════════════
    ...
    # Permite usar los valores de la calculadora O ingresar nuevos
